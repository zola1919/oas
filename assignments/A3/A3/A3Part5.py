...
if __name__ == "__main__":    
    ...
    f1=110
    M=512
    x=genSine(1,f1, 1, fs, M/fs)
    mX1_80, mX2_80, mX3_80 = zpFFTsizeExpt(x, fs)
    samples1=np.arange(80)
    samples2=np.arange(160,step=2)
    plt.figure(figsize=(10, 6))
    plt.plot(samples2,mX1_80, label='Case 1 (256 samples), window w1 (256 samples), and FFT size of 256')
    plt.plot(samples1,mX2_80, label='Case 2 (512 samples), window w2 (512 samples), and FFT size of 512')
    plt.plot(samples1,mX3_80, label='Case 3 (256 samples with zero-padding), window w1 (256 samples), and FFT size of 512')
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(mX1_80, label='Case 1 (256 samples), window w1 (256 samples), and FFT size of 256')
    plt.plot(mX2_80, label='Case 2 (512 samples), window w2 (512 samples), and FFT size of 512')
    plt.plot(mX3_80, label='Case 3 (256 samples with zero-padding), window w1 (256 samples), and FFT size of 512')
    plt.xlabel('Frequency Bin Index')
    plt.ylabel('Magnitude Spectrum (dB)')
    plt.legend()
    plt.grid()
