...
def suppressFreqDFTmodel(x, fs, N):
    ...
    ## Your code here
    mX, pX = dftAnal(x, w, N)
    bin_70Hz = int(np.ceil(70 * N / fs))
    print(mX[:bin_70Hz+5])
    mX_filter=mX.copy()
    mX_filter[:bin_70Hz] = -120
    print(mX_filter[:bin_70Hz+5])
# compute the inverse dft of the spectrum
    y = dftSynth(mX, pX, w.size)*outputScaleFactor
    y_filter= dftSynth(mX_filter, pX, w.size)*outputScaleFactor
    # create figure
    ...
    plt.figure()
    plt.plot(float(fs)*np.arange(mX.size)/float(N), mX_filter, 'r')
    plt.axis([0,80,-130, 0])
    plt.title ('magnitude spectrum: mX_filter')
    plt.ylabel('amplitude (dB)')
    plt.xlabel('frequency (Hz)')
    ...
    plt.xlabel('time (sec)')
    plt.title('output sound: y(using just the window function)')
    
    
    plt.figure()
    plt.plot(np.arange(M)/float(fs), y_filter)
    plt.axis([0, M/float(fs), min(x), max(x)])
    plt.ylabel('amplitude')
    plt.xlabel('time (sec)')
    plt.title('output sound: y_filtered')
    
    
    plt.figure()
    plt.plot(np.arange(M)/float(fs), x)
    plt.axis([0, M/float(fs), min(x), max(x)])
    plt.ylabel('amplitude')
    plt.xlabel('time (sec)')
...
if __name__ == "__main__":    
    ...
    x=genSine(1,f1, 1, fs, M/fs)+genSine(1,f2, 1, fs, M/fs) + genSine(1,f3, 1, fs,M/fs)+genSine(1,f4, 1, fs, M/fs)
