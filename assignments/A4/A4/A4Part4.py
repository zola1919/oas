...
import math
from A4Part3 import computeEngEnv,plot_spectrogram
...
def computeODF(inputFile, window, M, N, H):
    ...
    ### your code here
    # compute the energy envelopes because we need the energy on the band frequencies
    xmX, low_env, high_env = computeEngEnv(inputFile, window, M, N, H)
    
    # initialize the ODF arrays
    ODF_low = np.zeros(len(low_env))
    ODF_high = np.zeros(len(high_env))
    
    i = 1
    while i < len(low_env):
        # compute the difference between consecutive values
        ODF_low[i] = low_env[i] - low_env[i - 1]
        ODF_high[i] = high_env[i] - high_env[i - 1]
        # increase the index
        i += 1
    
    # rectify the ODF
    ODF_low_rectified = np.where(ODF_low > 0, ODF_low, 0)
    ODF_high_rectified = np.where(ODF_high > 0, ODF_high, 0)
    
    # append the ODF array
    ODF = np.array([ODF_low_rectified, ODF_high_rectified])
...
xmX, ODF = computeODF(input_file, window, M, N, H)
fs, s = wavfile.read(input_file)
plot_spectrogram(xmX, M, N, H, fs, ODF[0], ODF[1], window, title="Onset Detection")
